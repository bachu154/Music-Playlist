...
class App extends Component {
  ...
  onDeleteTrack = id => {
    ...
    const updatedList = tracksList.filter(each => each.id !== id)
    this.setState({tracksList: updatedList})
  }

  getFilteredTracks = () => {
    const {searchInput, tracksList} = this.state
    return tracksList.filter(each =>
                ...
                <li className="song-item" key={each.id}>
                  <div className="song-info">
                    ...
                    <div className="text-info">
                      <p className="track-name">{each.name}</p>
                      <p className="track-genre">{each.genre}</p>
                    </div>
                  </div>
                  <div className="track-right">
                    <p className="track-duration">{each.duration}</p>
                    <button
                      type="button"
                      data-testid="delete"
                      className="delete-button"
                      onClick={() => this.onDeleteTrack(each.id)}
                    >
